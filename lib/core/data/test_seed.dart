...
/// DEPRECATED: Use demoMovimientos() from movimiento_repository.dart instead.
/// 
/// This function was used to seed test movimientos in SQLite, but the app now
/// uses InMemoryMovimientoRepository which loads data from demoMovimientos().
/// 
/// Seed test movements for the Movimientos screen.
/// Inserts 30 movements across 12 dates using the demo products and users.
/// Skips if the `movimientos` table already has data (idempotent).
@Deprecated('Use demoMovimientos() from movimiento_repository.dart instead')
Future<int> seedTestMovimientos() async {
  final db = await LocalDatabase.instance.database;
  final count = Sqflite.firstIntValue(
    await db.rawQuery('SELECT COUNT(*) FROM movimientos'),
  );
  ...
  // Helper to create a movement row map
  ...
  // ── Jul 12: Venta grande con3 productos ──
  rows.add(m(
    productoId: 'prod-arroz',
    productoNombre: 'Arroz Diana 1kg',
    usuarioId: '00000000-0000-4000-9000-000000000001',
  ...
  for (final row in rows) {
    await db.insert('movimientos', row);
  }
